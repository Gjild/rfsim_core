...
# --- CORRECTED IMPORTS ---
# 1. Fixed the relative import paths from '..' to '.' for sibling modules.
# 2. Added `Quantity` to the import from `..units` for type hinting.
...
class SimulationEngine:
    ...
    def _run_single_level_simulation(self, circuit: Circuit) -> Tuple[np.ndarray, Optional[DCAnalysisResults]]:
        ...
        try:
            ...
            dc_analysis_results = dc_analyzer.analyze()  # Returns DCAnalysisResults object

            topology_analyzer = TopologyAnalyzer(circuit, self.cache)
            topo_results = topology_analyzer.analyze()  # Returns TopologyAnalysisResults object
            ...
            # 3. MNA Assembly and Pre-computation (logic moved verbatim from previous phase).
            ...
            # 4. Main Frequency Loop for Assembly and Solving (logic moved verbatim).
    ...
    def _map_dc_y_to_ac_ports(self, dc_results: DCAnalysisResults, ac_port_names: List[str]) -> np.ndarray:
        ...
        # 1. Access data directly from the formal result object's attributes.
        y_dc_qty = dc_results.y_ports_dc
        
        # 2. Gracefully handle the case where there is no DC solution (e.g., all ports open).
        if y_dc_qty is None:
            return y_mapped
        ...
        # 3. Map the DC Y-matrix onto the AC port space.
        ...
        # 4. Handle AC ports that were shorted to the DC ground supernode.
        for ac_idx, ac_name in enumerate(ac_port_names):
            if ac_to_dc_idx_map.get(ac_idx) is None:
                # This check is now fully self-contained, using only the result object.
